...
const LifeCounter: React.FC<LifeCounterProps> = ({ imageUrl, value, onChange, title, borderColor, textColor = "text-white" }) => {
  return (
    <div className={`relative flex flex-col items-center justify-center bg-[#1e293b] rounded-xl border-2 ${borderColor} overflow-hidden h-20 md:h-32 w-full transition-all duration-300 shadow-lg`}>
      ...
      <div className="relative z-10 w-full h-full flex flex-col justify-between py-1">
        ...
        <div className="flex items-center justify-between px-2 flex-1">
          <button 
            ...
            className="bg-slate-700/80 hover:bg-slate-600 text-slate-300 hover:text-white w-8 h-8 md:w-10 md:h-10 rounded-full flex items-center justify-center transition active:scale-95 shadow-md"
          >
            <Minus size={16} strokeWidth={3} />
          </button>

          <span className={`text-4xl md:text-6xl font-black ${textColor} drop-shadow-2xl font-mono tracking-tighter`}>
            {value}
          </span>
